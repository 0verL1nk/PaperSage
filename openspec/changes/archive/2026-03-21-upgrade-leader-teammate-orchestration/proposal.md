...
- Keep the Leader as the only user-facing dialogue owner while surfacing structured handoff and scheduler hints to support stepwise team decisions.
...
- Surface scheduler convenience data through tool results so the Leader can decide when to dispatch, review, or replan without being bypassed by an automatic loop.
...
- `leader-paced-team-control`: Leader-owned pacing with structured handoff and scheduler hints instead of forced auto-dispatch.
...
- `todolist-middleware`: Extend todo state and dependency handling so team scheduling can operate on structured ready/blocked/failed task records and expose scheduler hints in tool results.
- `orchestration-middleware`: Change team activation from suggestion-only guidance to a structured handoff into the Leader-teammate orchestration flow while preserving Leader pacing and dialogue ownership.
